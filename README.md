...
- **Pausar/reanudar una empresa** (p. ej. por impago): mientras está pausada, ni su jefe ni sus empleados pueden crear ni modificar nada (puestos, tarifas, turnos, incidencias, cambio de PIN) — solo consultar lo que ya existe. El jefe y los empleados ven un aviso explicándolo. Se aplica con políticas RLS (bloquean insert/update/delete, nunca el select) más una comprobación en la Edge Function que da de alta usuarios nuevos.
